...
fs.readdirSync("./config/validation")
  .filter((file) => {
    return (
      file.indexOf(".") !== 0 &&
      file !== basename &&
  ...
  .forEach((file) => {
    ...
    validator.addSchema(
      require("../../config/validation/" + file),
      schema_name
    );
  });
...
const error_response = ([error]) => {
  ...
  if (error.keyword === "enum") {
    enumStr =
      " " + error.params.allowedValues.map((val) => `"${val}"`).join(`, `);
  }
  return {
    error: {
...
module.exports = (schemaName) => async (req, res, next) => {
  ...
  if (schema.$async) {
    ...
    if (!valid)
      return res.status(400).json(error_response(validator.errors)).end();
  }
  return next();
};
